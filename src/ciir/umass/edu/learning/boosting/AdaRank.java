...
public class AdaRank extends Ranker {
		...
		for(;t<=nIteration;t++)
		{
			...
			if(withEnqueue)
			{
				if(bestWR.getFID() == lastFeature)//this feature is selected twice in a row
				{
					...
					bestScoreOnValidation = 0.0;//no best model just yet
	...
	public void learn()
	{
		...
		if(trainWithEnqueue)
		{
			...
			{
				PRINTLN("---------------------------");
				PRINTLN("Retrain without enqueue...");
				PRINTLN("--------------------------------------------------------");
				PRINTLN(new int[]{7, 8, 9, 9, 9}, new String[]{"#iter", "Sel. F.", scorer.name()+"-T", scorer.name()+"-V", "Status"});
				PRINTLN("--------------------------------------------------------");
				
				List<WeakRanker> tmpR = new ArrayList<WeakRanker>();
				List<Double> tmpW = new ArrayList<Double>();
				double tmpScore = bestScoreOnValidation;
				if(validationSamples != null && bestModelRankers.size()>0)
				{
					tmpR.addAll(bestModelRankers);
					tmpW.addAll(bestModelWeights);
				}
				init();
				learn(1, false);
				if(tmpScore > bestScoreOnValidation)
				{
					bestScoreOnValidation = tmpScore;
					bestModelRankers.clear();
					bestModelWeights.clear();
					bestModelRankers.addAll(tmpR);
					bestModelWeights.addAll(tmpW);
				}
			}*/
		}
		else
	...
	public RankList rank(RankList rl)
	{
		...
		for(int i=0;i<rl.size();i++)
			for(int j=0;j<rankers.size();j++)
				score[i] += rweight.get(j) * rl.get(i).getFeatureValue(rankers.get(j).getFID());
