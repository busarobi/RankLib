...
public class RankBoost extends Ranker {
	...
	private double estimateTrainingError()
	{
		int count = 0;
		double e = 0.0;
		for(int i=0;i<samples.size();i++)
		{
			...
			for(int j=0;j<rl.size()-1;j++)
			{
				...
				{
					if(rl.get(j).getLabel() > rl.get(k).getLabel())//crucial pairs
						if(m[k] > m[j])
							count++;
				}
			}
		}
		e = ((double)count)/totalCorrectPairs;
		return e;
	}
	...
	public void init()
	{
		...
		else
		{
			...
			for(int i=0;i<features.length;i++)
			{
				double step = (Math.abs(fmax[i])-Math.abs(fmin[i]))/nThreshold;
		...
		for(int i=0;i<features.length;i++)
			tSortedIdx[i] = Sorter.sort(thresholds[i], false);
	...
	public RankList rank(RankList rl)
	{
		...
		for(int i=0;i<rl.size();i++)
			for(int j=0;j<wRankers.size();j++)
				score[i] += rWeight.get(j) * wRankers.get(j).score(rl.get(i));
