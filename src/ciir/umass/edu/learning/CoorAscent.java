...
public class CoorAscent extends Ranker {
	...
	public void learn()
	{
		...
		//double origScore = scorer.score(samples);//score according to the selected metric
		...
		PRINTLN("---------------------------");
		scorer.score(rank(samples));//compute all the score used as cached during the training process
		for(int r=0;r<nRestart;r++)
		{
			PRINTLN("[+] Random restart #" + (r+1) + "/" + nRestart + "...");
			double startingScore = bestScore;
			...
			while((weight.length>1&&consecutive_fails < weight.length - 1) || (weight.length==1&&consecutive_fails==0))
			{
					...
					for(int s=0;s<sign.length;s++)//search by both increasing and decreasing
					{
						...
						for(int j=0;j<nMaxIteration;j++)
						{
							...
							if(score > bestScore)
							{
								...
								String bw = ((bestWeightValue>0.0)?"+":"") + SimpleMath.round(bestWeightValue, 2);
				...
				if(bestScore - startingScore < tolerance)
					break;
			}
		}
		copy(bestWeight, weight);
	...
	public String model()
	{
		String output = "## Coordinate Ascent\n";
