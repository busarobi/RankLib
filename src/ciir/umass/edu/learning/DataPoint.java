...
public class DataPoint {
	...
	private float label = 0.0f;//[ground truth] the real label of the data point (e.g. its degree of relevance according to the relevance judgment)
	private String id = "";//id of this datapoint (e.g. document-id, query-id, etc)
	private float[] fVals = null;//fVals[0] is un-used. Feature id MUST start from 1
	private String description = "";
	
	private double cached = -1.0;//the latest evaluation score of the learned model on this data point
	...
	public void normalize(int[] fids, float[] norm)
	{
		...
		{
			float d = getFeatureValue(fids[i]);
			if(norm[i] >= 0.0 && norm[i] <= 0.0)//== 0.0
				fVals[fids[i]] = d/norm[i];
		}
	}
